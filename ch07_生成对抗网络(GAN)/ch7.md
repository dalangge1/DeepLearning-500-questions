...
![model collapsing](/assets/ch7/model_collpsing.png)
...
![](/assets/ch7/pixRNN.png)

![](/assets/ch7/pixCNN.png)
...
![CGAN网络结构](/assets/ch7/CGAN网络结构.png)
...
网络结构图如下：
![CGAN网络结构图](/assets/ch7/DCGAN结构图.png)
...
![DCGAN输入噪声理解](/assets/ch7/7.4.3.png)
此外，将几个输入噪声进行算数运算，可以得到语义上进行算数运算的非常有趣的结果。类似于word2vec。
![DCGAN输入噪声算术运算](/assets/ch7/7.4.3-2.png)
...
![Wass距离随迭代次数变化](/assets/ch7/Wass距离随迭代次数变化.png)
...
![LSGAN交叉熵与最小二乘损失对比图](/assets/ch7/lsgan loss compare.png)
...
![pix2pix结果示例](/assets/ch7/pix2pix结果示例.png)
...
​	拿左上角的分割图$\longrightarrow$街景图为例，语义分割图的每个标签比如“汽车”可能对应不同样式，颜色的汽车。那么模型学习到的会是所有不同汽车的评均，这样会造成模糊。![pix2pix语义地图L1loss结果](/assets/ch7/pix2pix语义地图L1loss结果.png)

**如何解决生成图像的模糊问题**？

​	这里作者想了一个办法，即加入GAN的Loss去惩罚模型。GAN相比于传统生成式模型可以较好的生成高分辨率图片。思路也很简单，在上述直观想法的基础上加入一个判别器，判断输入图片是否是真实样本。模型示意图如下：
![pix2pix模型示意图](/assets/ch7/pix2pix模型示意图.png)
...
![CycleGAN结果例子](/assets/ch7/CycleGAN结果例子.png)
...
![CycleGAN模型示意图](/assets/ch7/CycleGAN模型示意图.png)
...
![seqGAN模型](/assets/ch7/seqGAN模型.png)
...
![cycleGAN数据增广](/assets/ch7/cycleGAN数据增广.png)
